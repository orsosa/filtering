...
import subprocess
############## global setting ###########
outdir ="/volatile/clas12/users/osoto/sim/mix"
app ="/home/osoto/Clas12AnaHipo4/examples/particle_mix_tree"
script ="/home/osoto/dihadron/filtering/runscripts/run_sim_mix_pippim.sh"
WF ="pippim_sim"
DEBUG =False
track ="analysis"
FILELIST ="filelist/simGA_jan2020.txt"
...
def add_job(fname,wf,c=0):
    global outdir, app, script, FILELIST
    size = os.path.getsize(fname)/1024./1024.*1.15
    fn = fname.split("/")[-1]
    jname = wf + "_" + str(c)
    cmd  = "swif add-job -workflow " + wf + " -ram 600mb -project clas12 -disk " + "{0:.0f}".format(size) + "mb "
    cmd += " -name " + jname
    cmd += " -track " + track
    #cmd += " -cores 10"
    cmd += " -shell /bin/bash" 
